...
public class CrudServiceSupplier {
    ...
    private final String SQL_SELECT_BASE = "SELECT supplier_id, names, addres, city, phone_number, whatsapp_number, reference_number, email, admin_id FROM supplier WHERE states = 'A'";
    ...
    public Supplier mapRow(ResultSet rs) throws SQLException {
        ...
        supplier.setNames(rs.getString("names"));
        ...
        supplier.setAdmin_id(rs.getString("admin_id"));
    ...
    public void reactivate(int id) {
        try (Connection conn = AccessDB.getConnection();
            ...
            if (rowsAffected > 0) {
                System.out.println("Proveedor reactivado con ID: " + id);
            } else {
    ...
    public void update(Supplier bean) {
        try (Connection conn = AccessDB.getConnection();
            ...
            if (rowsAffected > 0) {
                System.out.println("Proveedor actualizado con ID: " + bean.getSupplier_id());
            } else {
    ...
    public void softDelete(int id) {
        try (Connection conn = AccessDB.getConnection();
            ...
            if (rowsAffected > 0) {
                System.out.println("Proveedor eliminado suavemente con ID: " + id);
            } else {
                System.out.println("No se encontró ningún proveedor con el ID proporcionado: " + id);
            }
